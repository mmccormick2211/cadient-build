...
choco config set --name="proxy" --value=$env:chocolateyProxyLocation
choco config set --name="proxyUser" --value=$env:chocolateyProxyUser
choco config set --name="proxyPassword" --value=$env:chocolateyProxyPassword

'############### Chocolatey Package Manager Feature Block ###############'
## ## Defaults are listed and commented out. Copy line, uncomment, and modify.
